...
Use `launcher-output` when a response starts or returns Primary or Frontier launchers. It reads `dispatchChannel`. `agent_thread_spawn` and `one_click` may pass without human paste instructions. `manual_paste` must include copyable fenced `prompt` blocks and left-sidebar paste guidance; file links and `Get-Content` commands are not substitutes.
