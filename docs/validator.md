...
- `formal-report`: readable status report with `Response ID`, `Response log path`, standard `Item` or `报告项` table header, stable role-aware table rows, numeric progress, basis, evidence details, a recommended next step, and no shell command dump. `Checkpoint` rows are rejected.
- `frontier-contract`: Frontier prompt or report text with B2 lane-local authority, active B0/B1/B2 closure rules, gap decision matrix, branch return gate, worktree decision, subagent-first current-thread dispatch, child ledger, recommended next step, fallback-only child launcher rules, and a JSON `openacp-frontier-orchestration-contract.v1` block.
