...
- `formal-report`: readable status report with `Response ID`, `Response log path`, standard `Item/Status` or `类型/状态` table header, stable role-aware table rows, numeric progress, basis, evidence details, a recommended next step, and no shell command dump. Chinese chat reports keep the `类型/状态` header and use first-column label padding to keep it on one line. `Checkpoint` rows are rejected.
